function [a, b, water, min_loss] = best_abw(dis, z1, z2)
    ...
    for i = 1:maxP
        as(i) = unifrnd(1, 2);
        bs(i) = unifrnd(0, 1);
        waters(i) = unifrnd(1, 10);
        losses(i) = get_loss(as(i), bs(i), waters(i), dis, z1, z2);
    end
    ...
    for gene = 1:30
        ...
        for i = 1:length(as)
            for j = i:length(as)
                if rand() < 0.1
                    ...
                    new_w = [new_w, waters(i) * ratio(3) + waters(j) * (1-ratio(3))];
                end
            end
        end
        ...
        for i = 1:length(as)
            if rand() < 0.5
                ...
                new_w = [new_w, waters(i) + unifrnd(-2, 2)];
            end
        end
        ...
        for i = 1:length(new_a)
            new_loss(i) = get_loss(new_a(i), new_b(i), new_w(i), dis, z1, z2);
        end
