function output = get_y2(k,l,lambda)

% k = 0.819847959;
% l=7;
% lambda=0.405;
% params =[-4,      -3.4,    2,    1,  -0.2,     -1.120816020E-02	,7.935337730E-03];
% 柯西色散定理
syms u v x center_x center_y a b d;
% n_in = u + v/(lambda^2);
...
% ---------------------------------出射光线--------------------------------
y = k*x;
ell_equation = ((x-center_x)^2) / (a^2) + ((y-(center_y+d))^2) / (b^2) - 1;
...
normal_inter = (a^2 * (y_inter - (center_y+d))) / (b^2 * (x_inter - center_x)); %法线斜率
...
ell_equation = ((x_1-center_x)^2) / (a^2) + ((y_1-(center_y+d))^2) / (b^2) - 1;
eqn = [ell_equation, line_equation];
sol = solve(eqn, [x_1,y_1]);
% 提取交点坐标
x_intersection = sol.x_1(1);
y_intersection = sol.y_1(1);
normal_inter_in = (a^2 * (y_intersection - (center_y+d))) / (b^2 * (x_intersection - center_x)); %法线斜率
...
% disp(double(subs(y_intersection_last,[center_x, center_y,a,b,d,u,v],params)));
